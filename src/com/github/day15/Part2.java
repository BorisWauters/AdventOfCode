...
public class Part2 {
    ...
    public static void main(String[] args) {
        try (Stream<String> stream = Files.lines(Paths.get("input/day15.txt"))) {
    ...
    private static void solve(String[] lines) {
        ...
        Point[][] bigCave = new Point[height * 5][width * 5];
        //Use small cave to create big cave
        for (int i = 0; i < height; i++) {
            for (int j = 0; j < width; j++) {
                for (int k = 0; k < 5; k++) {
                    int bigI = height * k + i;
                    for (int l = 0; l < 5; l++) {
                        ...
                        int risk = ((cave[i][j].risk - 1 + k + l) % 9) + 1;
                        Point point = new Point(bigI, bigJ, risk);

                        bigCave[bigI][bigJ] = point;
                        vertices.add(point);
                    }
                }
            }
